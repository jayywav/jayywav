...
## Combined request-flow update
The Recording section now appears in this order:
1. HOW BOOKING WORKS
2. REQUEST A SESSION — 2 / 3 / 4 hour package cards
3. WHEN DO YOU WANT TO RECORD? — request form

The request form no longer depends on the unreliable iPhone `sms:?body=` behavior.

New send flow:
1. Customer fills the form.
2. PREPARE SESSION REQUEST generates the full standardized request on the page.
3. SEND REQUEST uses the phone's native Share Sheet when supported.
4. COPY REQUEST is always available.
5. TEXT JAYY.WAV opens the plain SMS thread with (678) 551-5333.
6. DM ON INSTAGRAM opens @_jayy.wav.
7. If an iPhone opens Messages without carrying the body, the customer can paste the already-copied/generated request instead.
